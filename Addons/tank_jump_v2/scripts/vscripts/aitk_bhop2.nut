...
tankJumpDir <- {};  // key = Tank entity index, value = [vx, vy]
if (Director.GetGameModeBase()=="survival"&&!Entities.FindByName(null, "bind_tkhop")) {SpawnEntityFromTable("logic_timer",{targetname="bind_tkhop",vscripts="aitk_bhop",RefireTime = 0.5 , OnTimer = "!caller,runscriptcode,Thinks()"});}
function Thinks()
{
    local i,ent;
    for(i=0;i<16;i++)
...
function GetDistance(dis1,dis2)//方法是GetDistance([0，0，0],[0，0，0]);
{
...
function GetClosetSurvivor(rp)//方法同上用[0，0，0]
{
    local surPos=[0,0,0];
...
function DisableButton(ent,button)//能够使不按下此按钮。
{
...
function Tankruncmd()
{
    ...
    for(i=0;i<16;i++)
    {
        ...
        if (tankJumpDir.rawin(entIndex)) {
            local curVel = GetVectorWithFloat(ent, "vel");
...
function ClientPush(ent)
{
    ...
    if (currentTime - lastJumpTime >= 15.0) {
        jumpCount = 0;
    }
    ...
    if ((currentTime - lastJumpTime >= 0.5) && jumpCount < maxJumpCount && isOnGround) {
        ...
        local angles = ent.EyeAngles();
        local yawRad = angles.y * PI / 180.0;

        if (speed < 120.0) {
            push_forward = 650.0;
            vz = 450.0;
            vx = push_forward * cos(yawRad);
            vy = push_forward * sin(yawRad);
